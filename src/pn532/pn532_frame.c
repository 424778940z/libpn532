...
#ifdef PN532_DEBUG
    char str[buffer_to_hex_string_len_calc(PN532_FRAME_MAX_LENGTH)] = {0};
    if ( buffer_to_hex_string(frame_buffer, PN532_FRAME_MAX_LENGTH, str, sizeof(str), NULL) )
        pn532_interface->log("Raw frame buffer: %s\n", str);
    else
        pn532_interface->log("Raw frame buffer: buffer_to_hex_string failed\n");
#endif
