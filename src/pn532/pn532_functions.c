...
{
    uint8_t buff[use_extended_frame ? PN532_FRAME_EXTENDED_INFO_MAX_LENGTH : PN532_FRAME_NORMAL_INFO_MAX_LENGTH];
    uint8_t* p_buff = buff;
    ...
    PN532_FRAME* frame_req =
        PackFrame(use_extended_frame ? PN532_FRAME_EXTENDED_INFO : PN532_FRAME_NORMAL_INFO, buff, p_buff - buff);
...
bool Diagnose(PN532_DIAG diag, uint8_t* diag_params, uint8_t diag_params_len)
{
    ...
    // exec
    if ( !ExecCommand(PN532_COMMAND_DIAGNOSE, buff, p_buff - buff, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
...
bool GetFirmwareVersion(PN532_FW_VER* fw_ver)
{
    ...
    // exec
    if ( !ExecCommand(
             PN532_COMMAND_GETFIRMWAREVERSION, buff, p_buff - buff, buff, &response_size, PN532_COMMAND_TIMEOUT
         ) )
        return false;
...
bool ReadRegister(uint16_t reg_addr, uint8_t* const reg_val)
{
    uint8_t buff[2];
    uint16_t response_size = 1;

    buff[0] = (uint8_t)(reg_addr >> 8);
    buff[0] = (uint8_t)(reg_addr);

    // exec
    if ( !ExecCommand(PN532_COMMAND_READREGISTER, buff, 1, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
...
bool WriteRegister(uint16_t reg_addr, const uint8_t* const reg_val)
{
    uint8_t buff[2];
    ...
    // exec
    if ( !ExecCommand(PN532_COMMAND_WRITEREGISTER, buff, 1, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
...
// RF communication
bool RFConfiguration(PN532_RFConfiguration_CfgItem cfg_item_type, uint8_t* data_cfg, uint16_t data_len)
{
    uint8_t buff[PN532_RFConfiguration_BUFF_SIZE];
    uint8_t* p_buff = NULL;
    uint16_t response_size = PN532_RFConfiguration_BUFF_SIZE;

    // sanity check
    switch ( cfg_item_type )
    {
    case PN532_RFConfiguration_CfgItem_RFField:
        if ( data_len != 1 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_VariousTimings:
        if ( data_len != 3 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_MaxRtyCOM:
        if ( data_len != 1 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_MaxRetries:
        if ( data_len != 3 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_AnalogSettings_106k_typeA:
        if ( data_len != 11 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_AnalogSettings_212k_424k:
        if ( data_len != 8 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_AnalogSettings_typeB:
        if ( data_len != 3 )
            return false;
        break;
    case PN532_RFConfiguration_CfgItem_AnalogSettings_212k_424k_848k_iso4443dash4:
        if ( data_len != 9 )
            return false;
        break;
    ...
    // prepare request
    p_buff = buff;
    p_buff[0] = cfg_item_type;
    p_buff++;
    memcpy(p_buff, data_cfg, data_len);
    p_buff += data_len;

    // exec
    if ( !ExecCommand(PN532_COMMAND_RFCONFIGURATION, buff, p_buff - buff, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
...
bool InListPassiveTarget(PN532_InListPassiveTarget_Params params, PN532_InListPassiveTarget_Results* results)
{
    ...
    // exec
    if ( !ExecCommand(
             PN532_COMMAND_INLISTPASSIVETARGET, buff, p_buff - buff, buff, &response_size, PN532_COMMAND_TIMEOUT
         ) )
        return false;
...
)
{
    ...
    // exec
    if ( !ExecCommand(PN532_COMMAND_INDATAEXCHANGE, buff, p_buff - buff, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
...
bool InSelect()
{
    ...
    // exec
    if ( !ExecCommand(PN532_COMMAND_INSELECT, buff, 1, buff, &response_size, PN532_COMMAND_TIMEOUT) )
        return false;
